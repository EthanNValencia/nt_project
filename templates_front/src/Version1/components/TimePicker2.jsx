...
function TimePicker(props) {
  ...
  const [beginTimes, setBeginTimes] = useState(
    generateAvailableBeginTimes(appointmentsSchedule)
  );
  ...
  useEffect(() => {
    ...
    setRange(newRange);
    const newBeginTimes = generateAvailableBeginTimes(
  ...
  function findValidTime(times) {
    ...
    const minuteFilter = timeFilter[0].mins.filter(
      (min) => min.disabled == false
    );
    const firstValidMinute = minuteFilter[0];

    if (firstValidHour.beginHour == undefined) {
      return {
        hourValue: firstValidHour.endHour,
        minuteValue: firstValidMinute.min,
        hourIndex: firstValidHour.i,
        minuteIndex: firstValidMinute.i,
      };
    }
    return {
      hourValue: firstValidHour.beginHour,
      minuteValue: firstValidMinute.min,
      hourIndex: firstValidHour.i,
      minuteIndex: firstValidMinute.i,
    };
  ...
  function generateAvailableEndTimes() {
    ...
    if (begin.minuteValue >= 30) {
      obj = {
        endHour: beginTimes[beginHourIndex].beginHour,
        mins: copyMinsArray(beginTimes[beginHourIndex].mins),
        disabled: true,
        meredian: beginTimes[beginHourIndex].meredian,
        i: 0,
      };
    } else {
      obj = {
        endHour: beginTimes[beginHourIndex].beginHour,
        mins: copyMinsArray(beginTimes[beginHourIndex].mins),
        disabled: beginTimes[beginHourIndex].disabled,
        meredian: beginTimes[beginHourIndex].meredian,
        i: 0,
      };
    }
    ...
    if (
      begin.hourIndex < beginTimes.length &&
      beginTimes[beginHourIndex + 1] != undefined
    ) {
      obj = {
        endHour: beginTimes[beginHourIndex + 1].beginHour,
        mins: copyMinsArray(beginTimes[beginHourIndex + 1].mins),
        disabled: beginTimes[beginHourIndex + 1].disabled,
        meredian: beginTimes[beginHourIndex].meredian,
        i: 1,
      };
      endTimesArray.push(obj);
      ...
      obj = {
        endHour: beginTimes[beginHourIndex].beginHour + 1,
        mins: generateLastHourMinuteArray(),
        disabled: false,
        meredian: beginTimes[beginHourIndex].meredian,
        i: 1,
      };
  ...
  function validateTimes(endTimesArray) {
    for (let i = 0; i < endTimesArray.length; i++) {
      const minutes = endTimesArray[i].mins.filter(
        (min) => min.disabled == false
      );
      if (minutes.length == 0) {
  ...
  function modifyMinutes(endTimesArray) {
  ...
  function modifyMinutesDeprecated(endTimesArray) {
    // You need to redo this. It just isn't working.
    let min = 1;
    let max = 3;
    let counter = 0;
    for (let i = 0; i < endTimesArray.length; i++) {
  ...
  function onChangeEndHour(e) {
    ...
    const newEnd = {
      minuteValue: minute.min,
      minuteIndex: minute.i,
      hourValue: time.endHour,
      hourIndex: time.i,
    };
    setEnd(newEnd);
  }
  ...
  function onChangeBeginMinute(e) {
    const minuteFilter = beginTimes[begin.hourIndex].mins.filter(
      (min) => min.min == e
    );
  ...
  function onChangeEndMinute(e) {
    const minuteFilter = endTimes[end.hourIndex].mins.filter(
      (min) => min.min == e
    );
    const minute = minuteFilter[0];
          ...
          <div className="flex justify-between">
            <div>
              ...
              <label className="pr-6">
                {beginTimes[begin.hourIndex].meredian[0]}
              </label>
            </div>
            <div>
